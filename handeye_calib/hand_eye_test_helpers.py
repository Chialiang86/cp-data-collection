import misc.handeye_calib.transformations as tf
import numpy as np

from misc.handeye_calib.dual_quaternion_hand_eye_calibration import (
    compute_hand_eye_calibration, align_paths_at_index,
    compute_dual_quaternions_with_offset, HandEyeConfig)
from misc.handeye_calib.hand_eye_calibration_plotting_tools import (
    plot_alignment_errors, plot_poses)
from misc.handeye_calib.dual_quaternion import DualQuaternion
from misc.handeye_calib.quaternion import Quaternion


def random_quaternion():
    quaternion = tf.random_quaternion()
    if quaternion[3] < 0.0:
...
def random_rotation():
    return tf.random_rotation_matrix()


def random_translation():
    translation = tf.random_vector(3)
    translation_transformation = tf.translation_matrix(
        translation)
...
def generate_test_path(n_samples, include_outliers=False,
    ...
    for i in range(n_samples):
        q_tmp = tf.quaternion_from_euler(
